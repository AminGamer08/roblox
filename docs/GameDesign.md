...
* **ServerScriptService**
  ...
  * `ChallengeArenaController` clones arena models, tracks goal touches, and cleans up hazards.
  * `RewardService` computes payouts, tracks streaks, and handles DataStore persistence.
  * `RemotesSetup` / `RewardBootstrap` bootstrap RemoteEvents and module lifecycles automatically on server start.
* **ReplicatedStorage**
...
3. Goal pads report finishing times back to the server; hazards eliminate players who fall behind.
4. When the challenge ends, the server calculates rankings, awards Clout Points, updates persistence, and resets streaks.
5. A short intermission teleports everyone to the lobby before the next rotation.
